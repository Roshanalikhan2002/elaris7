...
foreach ($file in $files) {
    Write-Host "Re-enabling BOGO Slider and Cleanup in $($file.Name)..."
    $content = Get-Content $file.FullName -Raw
    
    # 1. Restore Section Display
    ...
    # 2. Update BOGO HTML block to 12 products
    # We find the whole <div class="bogo-inline-section">...</div> and replace it
    ...
    # 5. Fix Accordion Spacing
    # Add padding to summary
    $content = $content.Replace('.accordion-item {', '.accordion-item { border-top: 1px solid rgba(0,0,0,0.1);') # Add top border for clarity
    $content = $content.Replace('.accordion-item summary {', ".accordion-item summary {`n              padding: 24px 0;")
    $content = $content.Replace('padding: 18px 0;', 'padding: 12px 0;') # Reduce outer padding to transition to inner padding

    Set-Content -Path $file.FullName -Value $content
}

Write-Host "BOGO Restoration and Cleanup complete!"
